...
var RequestProcessingFactory = /** @class */ (function () {
    ...
    RequestProcessingFactory.authorized = function (handler) {
        return RequestProcessingFactory.unauthorized(function (req, res) {
            return __awaiter(this, void 0, void 0, function () {
                return __generator(this, function (_a) {
                    switch (_a.label) {
                        case 0: return [4 /*yield*/, passport_1.default.authenticate('jwt', { session: false }, function (err, user) {
                                try {
                                    if (err || !user) {
    ...
    RequestProcessingFactory.onlyModerator = function (handler) {
        return RequestProcessingFactory.userHasStatuses(handler, [status_enum_1.Status.Admin, status_enum_1.Status.Moderator], 'Нужны права модератора');
    };
    RequestProcessingFactory.onlyAdmin = function (handler) {
        return RequestProcessingFactory.userHasStatuses(handler, [status_enum_1.Status.Admin], 'Нужны права администратора');
    };
    RequestProcessingFactory.hasRelevantRefreshAndAccessTokens = function (handler) {
        return RequestProcessingFactory.unauthorized(function (req, res) {
            ...
            var access_token_payload = jsonwebtoken_1.default.decode(req.headers.authorization.replace('Bearer ', ''), { json: true });
            var refresh_token_payload = jsonwebtoken_1.default.decode(req.body.refresh_token, { json: true });
    ...
    RequestProcessingFactory.userHasStatuses = function (handler, statuses, message) {
        if (message === void 0) { message = 'Недостаточно прав'; }
        return RequestProcessingFactory.authorized(function (req, res, user) {
            if (!statuses.some(function (status) { return user.statuses.includes(status); })) {
